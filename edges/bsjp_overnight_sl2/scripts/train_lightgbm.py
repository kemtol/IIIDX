#!/usr/bin/env python3
"""
Train LightGBM for BSJP overnight objective with strict safeguards:
...
# When --feature-modules-dir is used, only these core columns are read from monolithic L2.
# Feature columns come from individual module parquets in modules_dir.
CORE_MODEL_COLS = [
    "date", "ticker",
    "entry_price", "exit_price", "overnight_return",  # execution/backtest
    "label_tp", "label_sl2", "label_name",            # labels/outcome
]

DEFAULT_TRAINING_PATH = DATAMART_DIR / "training_datamart_bsjp_overnight.parquet"
DEFAULT_OUTPUT_DIR = IDX_DIR / "model" / "BSJP" / "bsjp_overnight_v0"
DEFAULT_FEATURE_IMPORTANCE_PATH = IDX_DIR / "model" / "BSJP" / "bsjp_overnight_v0" / "feature_importance.csv"

TARGET_COL = "label_tp"
ID_COLS = {"date", "ticker"}
OUTCOME_COLS = {
    # BSJP overnight columns
    "entry_price",
    "exit_price",
    "overnight_return",
    "label_tp",
    "label_sl2",
    "label_name",
    # BPJS columns (kept for compatibility if mixed datamart)
    "entry_price_opening",
    "high_to_cutoff",
    "low_to_cutoff",
    "close_to_cutoff",
...
class PolicyConfig:
    ...
    rank_weights: list[float] | None = None  # e.g. [0.6, 0.3, 0.1] for rank-based skewed weight
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--rank-weights", type=str, default=None,
                        help="Comma-separated rank-based weights, e.g. '0.6,0.3,0.1'. Overrides score-proportional weighting.")
    ...
    parser.add_argument(
        "--enable-shap",
        action="store_true",
        help="Calculate SHAP values on OOT data and save summary.",
    )
    ...
    parser.add_argument(
        ...
        default="limit",
        help=(
            "'limit': fills at entry_price/exit_price with fixed broker cost (current default). "
            "'market': stresses with bid-ask spread cost based on IDX tick size rules "
    ...
    parser.add_argument("--lambda-l2", type=float, default=0.5)
    parser.add_argument("--min-gain-to-split", type=float, default=0.01)
    parser.add_argument(
        "--scale-pos-weight",
        type=float,
        default=-1.0,
...
def build_group_sizes(dates: pd.Series) -> list[int]:
    if len(dates) == 0:
        return []
    ds = pd.to_datetime(pd.Series(dates), errors="coerce").reset_index(drop=True)
    ds = ds.fillna(pd.Timestamp("1970-01-01"))
    return ds.groupby(ds, sort=False).size().astype(int).tolist()


def scores_to_prob(scores: np.ndarray) -> np.ndarray:
    z = np.asarray(scores, dtype=float)
    z = np.clip(z, -30.0, 30.0)
    return 1.0 / (1.0 + np.exp(-z))


def predict_signal(model: lgb.LGBMModel, X: pd.DataFrame, args: argparse.Namespace) -> np.ndarray:
    if args.model_objective == "ranker":
        return scores_to_prob(model.predict(X))
...
# ---------------------------------------------------------------------------
# IDX tick-size based bid-ask spread estimation
# ---------------------------------------------------------------------------
# IDX tick rules (fractional rules simplified):
#   Price range        Tick    Typical spread (ticks)
#   Rp 50 – Rp 200       1      2–3 ticks for penny stocks
#   Rp 200 – Rp 500      2      2 ticks
#   Rp 500 – Rp 2,000    5      1.5 ticks
#   Rp 2,000 – Rp 5,000 10      1.5 ticks
#   Rp 5,000+           25      1 tick (blue chips)
# We use a conservative multiplier (2.5× for <200, 2× for 200-500, 1.5× for >500).
# ---------------------------------------------------------------------------
_TICK_MULTIPLIER = {
    (0, 200):      (1,  2.5),
    (200, 500):    (2,  2.0),
...
def estimate_spread_frac(price: float) -> float:
    """Return one-way spread fraction (e.g., 0.01 = 1% per side).
    Total round-trip spread = entry_spread + exit_spread = 2× this value.
    """
    if price <= 0:
        return 0.0
    for (lo, hi), (tick, mult) in _TICK_MULTIPLIER.items():
        if lo <= price < hi:
            spread_pts = tick * mult
            return spread_pts / price
    return 0.0
...
def simulate_portfolio(
    ...
    for d, day in scored_df.groupby("date", sort=True):
        ...
        exec_cols_present = [c for c in EXECUTION_PRICE_COLS if c in day.columns]
        for c in exec_cols_present:
            day[c] = pd.to_numeric(day[c], errors="coerce")
        day = day.dropna(subset=["pred_proba"] + exec_cols_present)
        if "entry_price" in day.columns:
        ...
        if vix_threshold > 0.0 and "vix_prev_close" in day.columns:
            if day["vix_prev_close"].iloc[0] > vix_threshold:
        ...
        if policy.rank_weights is not None and len(policy.rank_weights) >= len(day):
            rw = np.array(policy.rank_weights[: len(day)], dtype=float)
            rw = rw / rw.sum()
        ...
        # BSJP: exit always at 09:05 T+1 (time-based), use overnight_return directly.
        # Overnight = market closed, TIDAK ADA kesempatan SL intraday.
        # sl_pct hanya label klasifikasi, BUKAN cut yang bisa dieksekusi.
        if "overnight_return" in day.columns:
            day["trade_gross_return"] = day["overnight_return"]
        else:
            # Fallback: BPJS intra-bar pessimistic fill
        ...
        # In "market" mode, apply bid-ask spread cost (buy on ask, sell on bid).
        if execution_model == "market":
            entry_col = "entry_price"
            exit_col = "exit_price"
            if entry_col in day.columns and exit_col in day.columns:
                entry_spread = day[entry_col].apply(estimate_spread_frac)
                exit_spread = day[exit_col].apply(estimate_spread_frac)
                spread_cost = entry_spread + exit_spread  # one-way each side
                day["trade_net_return"] = day["trade_gross_return"] - total_cost_frac - spread_cost
                day["spread_cost_frac"] = spread_cost
            else:
                day["trade_net_return"] = day["trade_gross_return"] - total_cost_frac
                day["spread_cost_frac"] = 0.0
        else:
            day["trade_net_return"] = day["trade_gross_return"] - total_cost_frac
        ...
        # Compound capital for next day's position sizing.
        # Without this, every day allocates from the initial capital_idr (flat),
        # but cumulative_net_return (computed via cumprod below) assumes reinvestment.
        capital_idr = capital_idr * (1.0 + net_day)
...
def train_lgbm(
    ...
    if args.oversample_ratio > 0.0:
        if args.model_objective == "ranker":
            raise ValueError("oversample_ratio is not supported in ranker mode (would break group structure).")
    ...
    if args.model_objective == "ranker":
        train_group = build_group_sizes(train_dates)
        valid_group = build_group_sizes(valid_dates)
...
def load_features_from_modules(modules_dir: Path, core_df: pd.DataFrame) -> pd.DataFrame:
    """
    Load feature module parquets from ``modules_dir`` and LEFT JOIN onto core_df.

    Each ``*_features.parquet`` file must have grain ``(date, ticker)`` (stock-level)
    or ``(date,)`` (market-level, e.g. global indices).  Modules are merged in
    alphabetical file order.

    **Column name conflicts:** if two modules define the same feature column name,
    pandas will silently add ``_x`` / ``_y`` suffixes (the later module wins with
    ``_y``).  This function warns on any columns that already exist before merge.

    Returns the augmented DataFrame with all module feature columns added.
    """
    module_files = sorted(modules_dir.glob("*_features.parquet"))
    if not module_files:
        print(f"[Modules] WARNING: no *_features.parquet found in {modules_dir}")
        return core_df
    ...
    for fpath in module_files:
        ...
        new_feats = set(module.columns) - set(on)
        existing_feats = set(df.columns) - set(on)
        ...
        n_feat = len(module.columns) - len(on)
        print(f"[Modules] {fpath.name}: {len(module):,} rows, {n_feat} feats, "
              f"merge_on={on}, core_rows={before} -> {len(df)}")
    return df
...
def main() -> None:
    ...
    if args.feature_modules_dir:
        # Modular loading: read only core columns from monolithic, augment with feature modules
        df = pd.read_parquet(args.training_path, columns=CORE_MODEL_COLS)
        df = load_features_from_modules(args.feature_modules_dir, df)
        print(f"[Init] feature modules loaded from {args.feature_modules_dir}")
    else:
        df = pd.read_parquet(args.training_path)
    if TARGET_COL not in df.columns:
        fail_with_reason(args.output_dir, "MISSING_TARGET_COLUMN", {"target": TARGET_COL})
    # For BSJP: overnight_return is sufficient. For BPJS: need entry/high/low/close.
    has_bsjp_cols = "overnight_return" in df.columns
    has_bpjs_cols = all(c in df.columns for c in ["entry_price_opening", "high_to_cutoff", "low_to_cutoff", "close_to_cutoff"])
    if not has_bsjp_cols and not has_bpjs_cols:
        fail_with_reason(args.output_dir, "MISSING_EXECUTION_COLUMNS", {"columns": EXECUTION_PRICE_COLS})
    if args.tp_pct <= 0:
    ...
    for idx, (tr_days, va_days) in enumerate(wf_splits, start=1):
        tr = pre_oot_df[pre_oot_df["date"].isin(tr_days)].copy().sort_values(["date", "ticker"]).reset_index(drop=True)
        va = pre_oot_df[pre_oot_df["date"].isin(va_days)].copy().sort_values(["date", "ticker"]).reset_index(drop=True)
        X_tr, y_tr = tr[feature_cols], tr[TARGET_COL]
        X_va, y_va = va[feature_cols], va[TARGET_COL]

        model_fold = train_lgbm(
            X_tr,
            y_tr,
            X_va,
            y_va,
        ...
        proba_va = predict_signal(model_fold, X_va, args)
    ...
    for mode in policy_modes:
        ...
        for p_cut in cut_values:
            for k in k_grid:
                for w in w_grid:
                    rw = [float(x) for x in args.rank_weights.split(",")] if args.rank_weights else None
                    policy = PolicyConfig(
                        mode=mode,
                        p_cut=float(p_cut),
                        max_positions=int(k),
    ...
    core_df = pre_oot_df[pre_oot_df["date"].isin(core_days)].copy().sort_values(["date", "ticker"]).reset_index(drop=True)
    es_df = pre_oot_df[pre_oot_df["date"].isin(es_days)].copy().sort_values(["date", "ticker"]).reset_index(drop=True)
    ...
    final_model = train_lgbm(
        ...
        train_dates=core_df["date"],
        valid_dates=es_df["date"],
        args=args,
    )

    train_prob = predict_signal(final_model, core_df[feature_cols], args)
    oot_prob = predict_signal(final_model, oot_df[feature_cols], args)
    ...
    if getattr(args, "enable_shap", False):
        print("[SHAP] Calculating SHAP values for OOT data...")
        try:
            import shap
            import matplotlib.pyplot as plt
            import warnings
            warnings.filterwarnings("ignore")
            # Calculate SHAP on a manageable sample of OOT to avoid memory explosion
            shap_sample = oot_df[feature_cols].sample(n=min(5000, len(oot_df)), random_state=42)
            explainer = shap.TreeExplainer(final_model.booster_)
            shap_values = explainer.shap_values(shap_sample)
            
            # If binary classification, shap_values is a list of [negative, positive]. We want positive class [1].
            ...
            shap.summary_plot(shap_vals_to_plot, shap_sample, show=False, max_display=20)
            plt.savefig(args.output_dir / "shap_summary.png", bbox_inches="tight")
            plt.close()
            ...
            shap_df.to_csv(args.output_dir / "shap_importance.csv", index=False)
            print("[SHAP] SHAP importance CSV saved.")
        except ImportError:
            print("[SHAP] WARNING: 'shap' or 'matplotlib' library not installed. Skipping SHAP analysis.")
        except Exception as e:
            print(f"[SHAP] Error calculating SHAP: {e}")
    ...
    metrics_payload = {
        ...
        "model": {
            "type": "lightgbm.LGBMRanker" if args.model_objective == "ranker" else "lightgbm.LGBMClassifier",
            ...
            "params": {
                ...
                "scale_pos_weight": (
                    args.scale_pos_weight if args.scale_pos_weight > 0 else "auto"
                ) if args.model_objective == "binary" else "not_used_in_ranker",
        ...
        "cost_assumptions": {
            ...
            "execution_model": args.execution_model,
            "note": (
                "'limit': fills at entry_price/exit_price with fixed broker cost. "
                "'market': adds IDX tick-based bid-ask spread per pick (buy on ask, sell on bid)."
            ),
        },
        "execution": {
    ...
    # Auto-generate PnL charts (20d / 50d / 100d)
    try:
        _plot_dir = Path(__file__).resolve().parents[3] / "pipeline" / "plot"
        if str(_plot_dir) not in sys.path:
            sys.path.insert(0, str(_plot_dir))
        from plot_pnl import generate_pnl_charts
        generate_pnl_charts(args.output_dir)
    except Exception as e:
        print(f"[plot] WARNING: chart generation failed — {e}")
